...
public class BaseTranslator {
    ...
    public static int[] convertBase(int[] digits, int baseA,
        ...
        if (baseA < 2 || baseB < 2 || precisionB < 1){
            ...
            int [] B_Array = new int[precisionB];
            int [] clone_digits = digits.clone();
            for (int i = 0; i < precisionB; i++){
                int carry = 0;
                for(int k = clone_digits.length - 1; k >= 0; k-- ){
                    if (clone_digits[k] < 0 || digits[k] >= baseA){
                        return null;
                    }
                    int x = clone_digits[k] * baseB + carry;
                    clone_digits[k] = x % baseA;
                    carry = x / baseA;
